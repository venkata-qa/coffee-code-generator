...
//       // Pick the "Reward" button within the selected card
...
//           } else {
//             cy.log(`Offer already claimed for ${name}.`);
//           }
//         });
//       });

//       cy.contains('h3', 'A hot or cold drink on us').parent().parent().within(() => {
//         cy.get('a[href*="octoplus/partner/offers/caffe-nero"]').then(($button) => {
//           if ($button.length > 0 && !$button.hasClass('disabled')) {
//             cy.wrap($button).click();
//             cy.log(`✅ Claimed the Hot Drinks Offer for ${name}.`);
//           } else {
...
//       cy.get("//span[contains(text(),'Activate offer')]").wait(3000).click();

//       cy.get("//h2[contains(text(),'Reward activated!')]").wait(5000).should("be.visible");
